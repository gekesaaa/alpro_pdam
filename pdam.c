...
int validasi4(){    //validasi untuk daya listrik kelas 2 klaster 1 dan klaster 2
	int angka;
    char karakter;
    if (scanf("%d%c",&angka,&karakter)!= 2 || angka < 450 || angka > 1300 || karakter != '\n'){
        fflush(stdin);
...
int validasi_listrik_subsidi(){   //validasi untuk daya listrik kelas 1 klaster 1
	int angka;
    char karakter;
    if (scanf("%d%c",&angka,&karakter)!= 2 || angka < 450 || angka > 900 || karakter != '\n'){
        fflush(stdin);
        printf("\t Input invalid! \n Masukkan data yang benar : ");
...
float tarif, bpm, denda, total_biaya;
